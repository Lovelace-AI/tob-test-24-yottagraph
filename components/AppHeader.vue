<template>
    <v-app-bar
        ...
        :style="{
            ...
            color: '#ffffff',
            '-webkit-app-region': isElectronMac ? 'drag' : 'no-drag',
            '-webkit-user-select': isElectronMac ? 'none' : 'auto',
            paddingLeft: isElectronMac ? '80px' : '0',
        }"
    >
...
<script setup lang="ts">
    ...
    // Check if we're in Electron on macOS
    const isElectronMac = computed(() => {
        return isElectron.value && isMacPlatform.value;
    });
