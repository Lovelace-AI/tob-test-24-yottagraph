...
<script lang="ts" setup>
    ...
    // Check if we're in the Electron login window
    const isElectronLoginWindow = computed(() => {
        return route.query.electron_login_window === 'true';
    });
    ...
    async function redirectToLogin() {
        ...
        let redirectUrl;
        if (isElectron.value) {
            // For Electron, use our custom protocol
            // This needs to be registered in Auth0 as an allowed callback URL
            redirectUrl = 'aether://auth/callback';

            // If we're in the login window, add the parameter to the state
            if (isElectronLoginWindow.value) {
                redirectUrl += '?electron_login_window=true';
            }
        } else {
            redirectUrl = `${useRequestURL().origin}/a0callback`;
        }

        // Add prompt=login to force Auth0 to show login page even if session exists
        ...
        console.log('[Login] Auth0 Login URL:', loginUrl);
        console.log('[Login] Redirect URL:', redirectUrl);
        console.log('[Login] Is Electron:', isElectron.value);
        console.log('[Login] Is Electron Login Window:', isElectronLoginWindow.value);

        if (isElectron.value) {
            // In Electron, we need to open this in the system browser
            // The webContents.setWindowOpenHandler will handle this
