...
export function useUserState() {
    async function clearUser() {
        ...
        if (isInElectron) {
            // Clear all local cookies through IPC
            if (window.electronAPI?.clearAuthCookies) {
                await window.electronAPI.clearAuthCookies();
            }

            // Navigate to login page
            // Note: We use prompt=login on the login page to force Auth0 to show the login screen
            await navigateTo('/login');
        } else {
            // For web, use the normal logout flow with Auth0
    ...
    async function setUserFromAuth0(code: string) {
        ...
        const isInElectron =
            typeof window !== 'undefined' && window.electronAPI?.isElectron === true;
        const redirectUrl = isInElectron
            ? 'aether://auth/callback'
            : `${useRequestURL().origin}/a0callback`;

        const url = useRuntimeConfig().public.auth0IssuerBaseUrl;

        let tokenResponse: any;

        // Check if we're in Electron to avoid CORS issues
        if (isInElectron && window.electronAPI?.auth0TokenExchange) {
            try {
                tokenResponse = await window.electronAPI.auth0TokenExchange(code, redirectUrl);
            } catch (ipcError) {
                console.error('[setUserFromAuth0] IPC token exchange failed:', ipcError);
                throw ipcError;
            }
        } else {
            // Web flow - use direct fetch
            ...
            try {
                tokenResponse = await $fetch<any>(`${url}/oauth/token`, {
                    method: 'POST',
        ...
        // Return the token data for Electron to store
